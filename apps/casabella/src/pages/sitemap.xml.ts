...
function renderUrl(u: UrlEntry): string {
  const alt = u.alternates
    .map((a) => `    <xhtml:link rel="alternate" hreflang="${escapeXml(a.locale)}" href="${escapeXml(a.url)}" />`)
    .join('\n');
