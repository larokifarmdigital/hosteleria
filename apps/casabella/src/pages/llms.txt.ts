...
/**
 * llms.txt — nuevo estándar emergente (2024-2025) que los LLMs consumen preferentemente.
 * Un markdown limpio con la información clave del sitio para que ChatGPT, Perplexity,
 * Claude, Gemini extraigan hechos y citen la web con precisión.
 *
...
export const GET: APIRoute = async () => {
  ...
  const resumen = t(restaurant.resumenIA, defaultLocale, defaultLocale) ||
    t(restaurant.seoDescripcion, defaultLocale, defaultLocale) ||
    '';

  // Horarios semanales
  const horarios = (restaurant.horariosSemana ?? []).map((d) => {
    const dia = DIAS_ES[d.dia] ?? d.dia;
    const turnos = (d.turnos ?? []).map((t) => `${t.apertura}–${t.cierre}`).join(', ');
  ...
  const dishesBlock = dishCategories.map((cat) => {
  ...
  // Vinos — solo nombres y regiones (menos verboso que platos)
  const winesBlock = wineCategories.map((cat) => {
    const nombreCat = t(cat.nombre, defaultLocale, defaultLocale);
    const items = wines
      .filter((v) => v.categoria?._id === cat._id)
      .map((v) => {
        const region = v.region ? ` (${v.region})` : '';
        const precio = v.precioCopa && v.precioBotella
  ...
  // FAQ
  const faqBlock = (restaurant.faq ?? []).map((item) => {
