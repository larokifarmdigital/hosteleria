...
/**
 * "Todos los idiomas activos del restaurante o ninguno":
 * si el editor rellena un campo i18n para algún idioma, debe rellenar el mismo
 * campo en todos los idiomas listados en `restaurante.idiomasActivos`. Si todos
 * están vacíos, es aceptable.
 *
...
export const validarTodosIdiomasOninguno = (r: Rule) =>
  r.custom<EntradaI18n[] | undefined>(async (entradas, ctx) => {
    ...
    return `Falta traducción en: ${faltan.join(', ')}. Si rellenas un idioma, debes rellenar todos los activos de este restaurante.`;
  });
