...
export const idioma = defineType({
  ...
  type: 'document',
  description:
    'Idioma disponible en el sistema. Cada restaurante escoge desde su campo "idiomasActivos" cuáles usa.',
  fields: [
    defineField({
      name: 'codigo',
    ...
    defineField({
      ...
      type: 'string',
      description: 'Cómo aparece en el Studio. Ej.: "Español", "English", "Català".',
  ...
  preview: {
    ...
    prepare({ title, subtitle }) {
      return { title: title ?? '—', subtitle: subtitle ? subtitle.toUpperCase() : '?' };
    },
  },
});
